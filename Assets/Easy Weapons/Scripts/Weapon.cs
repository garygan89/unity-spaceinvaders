...
public class Weapon : MonoBehaviour
{
	...
	void Fire()
	{
		...
		for (int i = 0; i < shotPerRound; i++)
		{
			...
			if (Physics.Raycast(ray, out hit, range))
			{
				...
				// Check whether we hit our "green guy"
				checkRaycastHitTarget(hit);
	...
	void checkRaycastHitTarget(RaycastHit hit) {
		if (hit.collider.gameObject.CompareTag ("Enemy")) {
			// change our "green guy" material to gray and drop it down to the floor
			hit.collider.GetComponent<EnemyController> ().KillEnemy ();
			//Debug.Log ("enemy!");
		} else {
			//Debug.Log ("not enemy!");
		}
